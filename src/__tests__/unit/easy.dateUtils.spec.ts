...
describe('getDaysInMonth', () => {
  ...
  it('유효하지 않은 월에 대해 적절히 처리한다', () => {
    // Date 객체의 자동 정규화(normalize) 동작으로 인해, 변환된 날짜의 마지막 일수를 반환함
    expect(getDaysInMonth(2024, 13)).toBe(31); // 2024-13 -> 2025-01
    expect(getDaysInMonth(2024, 14)).toBe(28); // 2024-14 -> 2025-02
    expect(getDaysInMonth(2024, 15)).toBe(31); // 2024-15 -> 2025-03
    expect(getDaysInMonth(2024, 16)).toBe(30); // 2024-16 -> 2025-04
    expect(getDaysInMonth(2024, 17)).toBe(31); // 2024-17 -> 2025-05
    expect(getDaysInMonth(2024, 18)).toBe(30); // 2024-18 -> 2025-06
    expect(getDaysInMonth(2024, 19)).toBe(31); // 2024-19 -> 2025-07
    expect(getDaysInMonth(2024, 20)).toBe(31); // 2024-20 -> 2025-08
    expect(getDaysInMonth(2024, 21)).toBe(30); // 2024-21 -> 2025-09
  });
});
...
describe('getWeekDates', () => {
  ...
  it('주의 끝(토요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const date = new Date('2024-11-09');
...
describe('getEventsForDay', () => {
  ...
  it('특정 날짜(1일)에 해당하는 이벤트만 정확히 반환한다', () => {
    expect(getEventsForDay(events, 1)).toEqual([events[0], events[2]]);
  });

  it('해당 날짜에 이벤트가 없을 경우 빈 배열을 반환한다', () => {
    expect(getEventsForDay(events, 14)).toEqual([]);
  });
  ...
  it('날짜가 32일 이상인 경우 빈 배열을 반환한다', () => {
    ...
    expect(getEventsForDay(events, 100)).toEqual([]);
  });
});
...
describe('formatDate', () => {
  ...
  it('day 파라미터가 제공되면 해당 일자로 포맷팅한다', () => {
    ...
    expect(formatDate(date, 1)).toBe('2024-12-01');
  });
